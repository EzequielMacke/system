...
<script>
   ...
   $(document).ready(function(){
        @foreach($articulo->setting_product->whereNotNull('raw_materials_id') as $materials)
            var articulo_id = @json($materials->raw_materials_id);
            var material = @json($materials->raw_material->description);
            var cantidad = @json($materials->quantity);
            var fila = '<tr><td><input type="hidden" name="materiales[]" value="' + articulo_id + '">' + material + '</td><td><input type="hidden" name="cantidades[]" value="' + cantidad + '">' + cantidad + '</td><td><a href="javascript:;" onClick="removeRow(this, ' + count + ');"><i style="font-size:17px;" class="fa fa-times"></i></a></td></tr>';
