@extends('layouts.AdminLTE.index')
@section('title', 'Insumos Utilizados')
@section('content')
<div class="row">
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            <div class="ibox-title">
                <div class="btn-group pull-right">
                    <a href="{{ url('input-used/create') }}" class="btn btn-primary btn-xs"><i class="fa fa-plus"></i> Agregar</a>
                </div>
            </div>
            <div class="ibox-content pb-0">
                <div class="row">
                    <form method="GET">
                        <div class="form-group col-md-2">
                            <input type="text" class="form-control" name="s" placeholder="Buscar..." value="{{ request()->s }}">
                        </div>
                    </form>
                </div>
            </div>
            <div class="ibox-content table-responsive no-padding">
                <table class="table table-hover table-striped mb-0">
                    <thead>
                        <tr>
                            <th>Nro° Registro de Insumo</th>
                            <th>Nro° de Orden de Servicio</th>
                            <th>Fecha de creación</th>
                            <th>Cliente</th>
                            <th>Obra</th>
                            <th>Descripción</th>
                            <th>Estado</th>
                            <th class="text-center">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($input_useds as $input_used)
                            <tr>
                                <td>{{ $input_used->id }}</td>
                                <td>{{ $input_used->order_id}}</td>
                                <td>{{ $input_used->date_created }}</td>
                                <td>{{ $input_used->client->razon_social }}</td>
                                <td>{{ $input_used->construction_site->description }}</td>
                                <td>{{ $input_used->description }}</td>
                                <td>
                                    <span class="label label-{{ config('constants.input_used_status_label.' . $input_used->status) }}">{{ config('constants.input_used_status.'. $input_used->status) }}</span>
                                </td>
                                <td class="text-center">
                                    @if($input_used->status == 1)
                                        <button type="button" class="btn btn-danger btn-xs change-status-button" data-id="{{ $input_used->id }}">Eliminar</button>
                                        <a href="{{ route('input_used.pdf', $input_used->id) }}" class="btn btn-primary btn-xs">Extraer PDF</a>
                                    @endif
                                </td>
                                <td class="text-center">
                                    {{-- <a href="{{ url('wish-service/' . $wishservice->id) }}"><i class="fa fa-info-circle"></i></a> --}}
                                    {{-- <a href="{{ url('wish-service/' . $wishservices->id . '/edit') }}"target="_blank" data-toggle="tooltip"><i class="fa fa-pencil"></i></a> --}}
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
            {{ $input_useds->appends(request()->query())->links() }}
        </div>
    </div>
</div>
@endsection
@section('layout_js')
    <script>
        $(document).ready(function() {
            // Manejar el clic en el botón de cambiar estado
            $(document).on('click', '.change-status-button', function() {
                var id = $(this).data('id');
                var confirmed = confirm('¿Estás seguro de que deseas cambiar el estado a eliminado?');
                if (confirmed) {
                    $.ajax({
                        url: '{{ url('input-used/change-status') }}/' + id,
                        method: 'GET',
                        success: function(response) {
                            if (response.success) {
                                location.reload();
                            } else {
                                alert('Error al cambiar el estado.');
                            }
                        },
                        error: function(xhr, status, error) {
                            console.error(error);
                            alert('Error al cambiar el estado.');
                        }
                    });
                }
            });
        });
    </script>
@endsection

